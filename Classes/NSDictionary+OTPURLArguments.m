//
//  NSDictionary+OTPURLArguments.h
//
...
@implementation NSDictionary (OTPURLArguments)

+ (NSDictionary *)otp_dictionaryWithHttpArgumentsString:(NSString *)argString {
  ...
  for (NSString *component in [components reverseObjectEnumerator]) {
    ...
    if (pos.location == NSNotFound) {
      ...
      key = [[component substringToIndex:pos.location]
              otp_stringByUnescapingFromURLArgument];
      val = [[component substringFromIndex:pos.location + pos.length]
              otp_stringByUnescapingFromURLArgument];
    }
    // otp_stringByUnescapingFromURLArgument returns nil on invalid UTF8
...
- (NSString *)otp_httpArgumentsString {
  NSMutableArray* arguments = [NSMutableArray arrayWithCapacity:[self count]];
  for (NSString *key in self) {
    [arguments addObject:[NSString stringWithFormat:@"%@=%@",
                                                    [key otp_stringByEscapingForURLArgument],
                                                    [[[self objectForKey:key] description] otp_stringByEscapingForURLArgument]]];
  }
